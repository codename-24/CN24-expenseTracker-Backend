...
let prem = async function ispremium(){
    ...
    await axios.get('http://54.215.236.204:3000/purchase/ispremium', { headers: {"Authorization" : token} })
    .then((response) => {
...
function addNewExpense(e){
    ...
    axios.post('http://54.215.236.204:3000/user/addexpense',expenseDetails, { headers: {"Authorization" : token} }).then((response) => {
...
window.addEventListener('load', ()=> {
    const token = localStorage.getItem('token');
    axios.get('http://54.215.236.204:3000/user/getexpenses', { headers: {"Authorization" : token} }).then(response => {
        if(response.status === 200){
...
function deleteExpense(e, expenseid) {
    const token = localStorage.getItem('token');
    axios.delete(`http://54.215.236.204:3000/user/deleteexpense/${expenseid}`, { headers: {"Authorization" : token} }).then((response) => {
